...
namespace QuanLyNhanSu.ThongKe
{
    public partial class tkSoNgayNghiKhongPhep : UserControl
    {
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (File.Exists(@"newSoNgayNghiKhongPhepReport.docx"))
                {
                    Process.Start(AppDomain.CurrentDomain.BaseDirectory + @"\newSoNgayNghiKhongPhepReport.docx");
                }
                else
                {
        ...
        private void btXem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt.Clear();
                dt = tkcl.tkSoNgayDiLamCuaNhanVien(ngaydau, ngaycuoi, 0);
        ...
        public void exportFile()
        {
            ...
            try
            {
                if (File.Exists(@"NghiKhongPhepReportTemplate.docx"))
                {
                    docX = CreateWordFromTemplate(DocX.Load(@"NghiKhongPhepReportTemplate.docx"));
                    docX.SaveAs(@"newSoNgayNghiKhongPhepReport.docx");
                }
                else
                {
        ...
        {
            template.AddCustomProperty(new CustomProperty("ReportTitle", "Báo cáo số ngày nhân viên nghỉ không phép"));
